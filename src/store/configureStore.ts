...
import { Action, NonNormalizedState, Reducer, State, StateHelpers } from '../interfaces';

// eslint-disable-next-line default-param-last
const generateReducer = <T extends keyof State>(reducer: Reducer<T>, initial: State[T]): Reducer<T> => (state = initial, action: Action) => {
  return reducer(state, action);
};

const generateReducers = <T extends keyof State>(newReducers: {[key in T]: Reducer<key>}, normalizedInitialState: State) => {
...
export default function configureStore(initialState: NonNormalizedState, helpersConfig: StateHelpers) {
