...
export default new Map([
  ...
  [ActionTypes.REQUEST_TAGS, state => update(state, {
    $merge: {
      isFetching: true
    }
  })
  ],
  [ActionTypes.RECEIVE_TAGS, (state, action) => update(state, {
    $merge: {
      isFetching: false,
      didInvalidate: false,
  ...
  [ActionTypes.POSTED_TAG_TO_RESTAURANT, (state, action) => update(state, {
    items: {
      entities: {
        tags: {
          [action.id]: {
