          ...
          result: {
            $apply: (result: number[]) => {
              ...
              const sortedResult = Array.from(result).sort((a, b) => {
                ...
                const restaurantA = getRestaurantById(
                  { restaurants: state },
                  a
                );
                const restaurantB = getRestaurantById(
                  { restaurants: state },
                  b
                );
              ...
              return sortedResult.some((r, i) => r !== result[i])
                ? sortedResult
                : result;
            },
          },
    ...
    case "RESTAURANT_DELETED": {
      return update(state, {
        ...
        items: {
          result: {
            $splice: [
              [getRestaurantIds({ restaurants: state }).indexOf(action.id), 1],
            ],
          },
        },
      });
                    ...
                    changedRestaurants[index] = update(changedRestaurant, {
                      $merge: {
                        tags: update(changedRestaurant.tags, {
                          $splice: [
                            [changedRestaurant.tags.indexOf(action.id), 1],
                          ],
                        }),
                      },
                    });
              ...
              $apply: (r) => {
                ...
                action.deselected.forEach((i) => {
                  ...
                  r[i.restaurantId] = {
                    ...r[i.restaurantId],
                    all_decision_count: maybeAddToString(
                      r[i.restaurantId].all_decision_count,
                      -1
                    ),
                  };
                });
