class ReportsController < ApplicationController
  ...
  # Your Account
  ACCOUNT_SUMMARY_WEB_FLAGS = [MembersService::ACCOUNT_SUMMARY]
  AUTHORIZATIONS_WEB_FLAGS = [MembersService::AUTHORIZATIONS]
  SETTLEMENT_TRANSACTION_ACCOUNT_WEB_FLAGS = [MembersService::STA_DETAIL_DATA]
  INVESTMENTS_WEB_FLAGS = [MembersService::INVESTMENTS]

  # Capital Stock
  CAPITAL_STOCK_ACTIVITY_WEB_FLAGS = [MembersService::CAPSTOCK_REPORT_ACTIVITY]
  CAPITAL_STOCK_TRIAL_BALANCE_WEB_FLAGS = [MembersService::CAPSTOCK_REPORT_TRIAL_BALANCE]
  CAPITAL_STOCK_AND_LEVERAGE_WEB_FLAGS = [MembersService::FHLB_STOCK_DATA]
  DIVIDEND_STATEMENT_WEB_FLAGS = [MembersService::CAPSTOCK_REPORT_DIVIDEND_STATEMENT]
  ...
  # Collateral
  BORROWING_CAPACITY_WEB_FLAGS = [MembersService::BORROWING_CAPACITY]
  ...
  ADVANCES_DETAIL_WEB_FLAGS = [MembersService::ADVANCES_DETAIL_HISTORY]
  INTEREST_RATE_RESETS_WEB_FLAGS = [MembersService::INTEREST_RATE_RESETS]
  LETTERS_OF_CREDIT_WEB_FLAGS = [MembersService::LETTERS_OF_CREDIT_DETAIL_REPORT]
  FORWARD_COMMITMENTS_WEB_FLAGS = [MembersService::FORWARD_COMMITMENTS]
  PARALLEL_SHIFT_WEB_FLAGS = [MembersService::PUTABLE_ADVANCE_PARALLEL_SHIFT]
  ...
  def letters_of_credit
    downloadable_report([:xlsx, :pdf]) do
      ...
      self.report_download_name = @as_of.blank? ? 'letters-of-credit' : "letters-of-credit-#{fhlb_report_date_numeric(@as_of)}"
  ...
  def account_summary
    ...
    downloadable_report(:pdf, nil, report_download_name) do
      ...
      if params[:job_id] || self.skip_deferred_load
        ...
        job_status = AccountSummaryJob.perform_later(current_member_id, request.uuid).job_status
        job_status.update_attributes!(user_id: current_user.id)
        @job_status_url = job_status_url(job_status)
        @load_url = reports_account_summary_url(job_id: job_status.id)
  ...
  def report_disabled?(report_flags)
    ...
    @report_disabled = member_info.report_disabled?(current_member_id, Array.wrap(report_flags))
  end
