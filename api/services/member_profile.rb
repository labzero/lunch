  ...
  module Services
    module MemberProfile
      ...
      def self.registered(app)
        service_root '/member_profile', app
        swagger_api_root :member_profile do
          api do
            key :path, '/{id}/member_position'
            operation do
              key :method, 'GET'
              key :summary, 'Retrieve member financial position'
              key :notes, 'Returns a row with member financial position'
              key :type, :MemberProfilePosition
              key :nickname, :getFinancialPositionForMember
              parameter do
                key :paramType, :path
                key :name, :id
                key :required, true
        ...
        relative_get '/:id/member_position' do
          member_id = params[:id]
          ...
          member_sta_cursor_string = <<-SQL
            ...
            AND  f.STA_ID = e.STA_ID AND e.fhlb_id = #{ActiveRecord::Base.connection.quote(member_id.to_i)}
            AND f.STX_UPDATE_DATE =
            (SELECT MAX(STX_UPDATE_DATE) FROM PORTFOLIOS.STA_TRANS WHERE FHLB_ID = 750)
          SQL
          ...
          if member_position_hash.count() > 0
            ...
            capital_stock_returns = MAPI::Shared::CommonMethod::capital_stock_requirements(total_cap_stock, advances_outstanding, mpf_unpaid_balance, mortgage_related_assets, settings.environment)
            if capital_stock_returns.length > 0
              capital_stock_hash = JSON.parse(capital_stock_returns)
              if capital_stock_hash.count() > 0
                stock_leverage = capital_stock_hash['additional_advances'].to_i
              end
            end
           end
          {
            sta_balance: member_sta_hash['STX_CURRENT_LEDGER_BALANCE'],
            credit_outstanding: member_position_hash['CREDIT_OUTSTANDING'],
            financial_available: member_position_hash['FINANCIAL_AVAILABLE'],
            stock_leverage:stock_leverage,
            collateral_market_value_sbc_agency: member_position_hash['SBC_MARKET_VALUE_AG'],
            collateral_market_value_sbc_aaa: member_position_hash['SBC_MARKET_VALUE_AAA'],
            collateral_market_value_sbc_aa: member_position_hash['SBC_MARKET_VALUE_AA'],
            borrowing_capacity_standard: member_position_hash['EXCESS_REG_BORR_CAP'],
            borrowing_capacity_sbc_agency: member_position_hash['EXCESS_SBC_BORR_CAP_AG'],
            borrowing_capacity_sbc_aaa: member_position_hash['EXCESS_SBC_BORR_CAP_AAA'],
            borrowing_capacity_sbc_aa: member_position_hash['EXCESS_SBC_BORR_CAP_AA']
          }.to_json
        end
      end
    end
