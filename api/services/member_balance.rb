    ...
    module MemberBalance
      ...
      def self.registered(app)
        ...
        relative_get "/:id/borrowing_capacity_details/:as_of_date" do
          ...
          if balance_hash.count > 0
            standard_breakdown_formatted = []
            std_breakdown.each do |row|
              reformat_hash = {'type' => row['COLLATERAL_TYPE'],
                      'count' => row['STD_COUNT'],
                      'original_amount' => row['STD_ORIGINAL_AMOUNT'].to_f.round,
                      'unpaid_principal' => row['STD_UNPAID_BALANCE'].to_f.round,
                      'market_value' => row['STD_MARKET_VALUE'].to_f.round,
                      'borrowing_capacity' => row['STD_BORROWING_CAPACITY'].to_f.round
              }
            ...
            sbc_breakdown = [
              { type: 'AA',
                total_market_value: balance_hash['SBC_MV_AA'].to_f.round,
                total_borrowing_capacity: balance_hash['SBC_BC_AA'].to_f.round,
                advances: balance_hash['SBC_ADVANCES_AA'].to_f.round,
                standard_credit: balance_hash['SBC_COVER_OTHER_AA'].to_f.round,
                remaining_market_value: balance_hash['SBC_MV_COLL_EXCESS_DEF_AA'].to_f.round,
                remaining_borrowing_capacity: balance_hash['SBC_COLL_EXCESS_DEF_AA'].to_f.round,
              },
              { type: 'AAA',
                total_market_value: balance_hash['SBC_MV_AAA'].to_f.round,
                total_borrowing_capacity: balance_hash['SBC_BC_AAA'].to_f.round,
                advances: balance_hash['SBC_ADVANCES_AAA'].to_f.round,
                standard_credit: balance_hash['SBC_COVER_OTHER_AAA'].to_f.round,
                remaining_market_value: balance_hash['SBC_MV_COLL_EXCESS_DEF_AAA'].to_f.round,
                remaining_borrowing_capacity: balance_hash['SBC_COLL_EXCESS_DEF_AAA'].to_f.round,
              },
              { type: 'Agency',
                total_market_value: balance_hash['SBC_MV_AG'].to_f.round,
                total_borrowing_capacity: balance_hash['SBC_BC_AG'].to_f.round,
                advances: balance_hash['SBC_ADVANCES_AG'].to_f.round,
                standard_credit: balance_hash['SBC_COVER_OTHER_AG'].to_f.round,
                remaining_market_value: balance_hash['SBC_MV_COLL_EXCESS_DEF_AG'].to_f.round,
                remaining_borrowing_capacity: balance_hash['SBC_COLL_EXCESS_DEF_AG'].to_f.round
              }
            ]
            ...
            {  date: as_of_date.to_date,
               standard: {
                 ...
                 excluded: {
                   blanket_lien: balance_hash['STD_EXCL_BL_BC'].to_f.round,
                   bank: balance_hash['STD_EXCL_BANK_BC'].to_f.round,
                   regulatory: balance_hash['STD_EXCL_REG_BC'].to_f.round
                  },
                 utilized: {
                   advances: balance_hash['STD_ADVANCES'].to_f.round,
                   letters_of_credit: balance_hash['STD_LETTERS_CDT_USED'].to_f.round,
                   swap_collateral: balance_hash['STD_SWAP_COLL_REQ'].to_f.round,
                   sbc_type_deficiencies: balance_hash['STD_COVER_OTHER_PT_DEF'].to_f.round,
                   payment_fees: balance_hash['STD_PREPAY_FEES'].to_f.round,
                   other_collateral: balance_hash['STD_OTHER_COLL_REQ'].to_f.round,
                   mpf_ce_collateral: balance_hash['STD_MPF_CE_COLL_REQ'].to_f.round
                }
               },
               sbc: {
                  collateral: sbc_breakdown,
                  utilized: {
                    other_collateral: balance_hash['SBC_OTHER_COLL_REQ'].to_f.round,
