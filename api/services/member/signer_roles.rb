      ...
      module SignerRoles
        def self.signer_roles(app, member_id)
          ...
          if app.settings.environment == :production
            ...
            SELECT SIGNERID, LOGIN_ID, LNAME, FNAME, FULLNAME, TITLE, ALLRNA, ALLPRODUCT, ADVSIGNER, COLLATSIGNER,MNYMKTSIGNER, SWAPSIGNER, SECURITYSIGNER, WIRESIGNER, REPOSIGNER, AFFORDABITYSIGNER, LASTUPDATED,  FHLB_ID
            FROM (
              SELECT SIGNERID, LOGIN_ID, LNAME, FNAME, FULLNAME, TITLE, ALLRNA, ALLPRODUCT, ADVSIGNER, COLLATSIGNER,MNYMKTSIGNER, SWAPSIGNER, SECURITYSIGNER, WIRESIGNER, REPOSIGNER, AFFORDABITYSIGNER, LASTUPDATED,  S.FHLB_ID, x.signer_id, nvl(x.token_active_status, 'N') Active_token_exists_flag
              FROM SIGNER.SIGNERS s LEFT JOIN (
                SELECT UNIQUE es.signer_id, token_active_status, LOGIN_ID
