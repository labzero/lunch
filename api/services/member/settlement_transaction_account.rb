      ...
      module SettlementTransactionAccount
        ...
        def self.current_sta_rate(app, member_id)
          ...
          if app.settings.environment == :production
            ...
            SELECT ACCOUNT_NUMBER, BALANCE, RATE, TRANS_DATE
            FROM PORTFOLIOS.STA_WEB_DETAIL
              WHERE FHLB_ID = #{ActiveRecord::Base.connection.quote(member_id)}
              AND  DESCR = 'Interest Rate / Daily Balance'
              AND TRANS_DATE =
            ...
            while row = current_sta_rate_cursor.fetch_hash()
              current_sta_data[:account_number] = row['ACCOUNT_NUMBER']
          ...
          current_sta_data[:account_number] = current_sta_data[:account_number].to_s if current_sta_data[:account_number]
