    ...
    module Member
      module MortgageCollateralUpdate
        ...
        INTEGER_FIELDS = %w(pledged_count updated_count depledged_count renumbered_count rejected_count accepted_count 
                            pledged_unpaid updated_unpaid depledged_unpaid rejected_unpaid renumber_unpaid accepted_unpaid 
                            total_count total_unpaid total_original pledged_original updated_original depledged_original 
                            rejected_original renumber_original accepted_original).freeze
        ...
        def self.mortgage_collateral_update(env, member_id)
          mcu_data = if env == :production
            mcu_query = <<-SQL
              SELECT 
                ...
                SYS_UPB_RENUMBER as renumber_unpaid,
                ...
                SYS_ORIG_AMOUNT_RENUMBER as renumber_original,
          ...
          if mcu_data
            processed_data = {
              date_processed: (Date.parse(mcu_data[:date_processed]) if mcu_data[:date_processed])
            }
            STRING_FIELDS.each do |field|
              processed_data[field.to_sym] = (mcu_data[field].to_s if mcu_data[field])
            end
            INTEGER_FIELDS.each do |field|
              processed_data[field.to_sym] = (mcu_data[field].round if mcu_data[field])
            end
