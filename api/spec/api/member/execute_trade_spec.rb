...
describe MAPI::ServiceApp do
  ...
  describe 'get_maturity_date' do
    it 'should add one day if overnight' do
      expect(MAPI::Services::Member::ExecuteTrade.get_maturity_date(Time.zone.parse('2015-02-03').to_date, 'overnight')).to eq(Time.zone.parse('2015-02-04').to_date)
    end
    it 'should add 7 days if 1week' do
      expect(MAPI::Services::Member::ExecuteTrade.get_maturity_date(Time.zone.parse('2015-02-03').to_date, '1week')).to eq(Time.zone.parse('2015-02-10').to_date)
    end
    it 'should add 1 months if 1month' do
      expect(MAPI::Services::Member::ExecuteTrade.get_maturity_date(Time.zone.parse('2015-02-03').to_date, '1month')).to eq(Time.zone.parse('2015-03-03').to_date)
    end
    it 'should add 1 year if 1year' do
      expect(MAPI::Services::Member::ExecuteTrade.get_maturity_date(Time.zone.parse('2015-02-03').to_date, '1year')).to eq(Time.zone.parse('2016-02-03').to_date)
    end
  end
  ...
  describe 'get_payment_info' do
    ...
    it 'should return overnight payment info' do
      expect(MAPI::Services::Member::ExecuteTrade.get_payment_info('overnight', 'whole', Date.today, Date.today)).to eq(overnight_response)
    end
    it 'should return open payment info' do
      expect(MAPI::Services::Member::ExecuteTrade.get_payment_info('open', 'whole', Date.today, Date.today)).to eq(open_response)
    end
    it 'should return whole loan 1 week payment info' do
      expect(MAPI::Services::Member::ExecuteTrade.get_payment_info('1week', 'whole', Date.today, Date.today)).to eq(whole_1_week_response)
    end
    it 'should return next month payment info' do
      expect(MAPI::Services::Member::ExecuteTrade.get_payment_info('1week', 'whole', Date.today, Date.today+32.days)).to eq(next_month_response)
    end
  end
  ...
  describe 'get_advance_rate_schedule' do
    ...
    it 'should return overnight advance rate schedule' do
      expect(MAPI::Services::Member::ExecuteTrade.get_advance_rate_schedule('overnight', rate, day_count, Date.today, Date.today)).to eq(overnight_response)
    end
    it 'should return week advance rate schedule' do
      expect(MAPI::Services::Member::ExecuteTrade.get_advance_rate_schedule('1week', rate, day_count, Date.today, Date.today)).to eq(week_response)
    end
  end
  ...
  describe 'get_advance_product_info' do
    ...
    it 'should return overnight advance product info' do
      expect(MAPI::Services::Member::ExecuteTrade.get_advance_product_info('overnight')).to eq(overnight_response)
    end
    it 'should return week advance product info' do
      expect(MAPI::Services::Member::ExecuteTrade.get_advance_product_info('1week')).to eq(week_response)
    end
  end

  describe 'Execute Trade' do
    let(:operation)  {'EXECUTE'}
    let(:execute_trade) { get "/member/#{MEMBER_ID}/execute_advance/#{operation}/#{amount}/#{advance_type}/#{advance_term}/#{rate}/#{signer}"; JSON.parse(last_response.body) }
  ...
  describe 'in the production environment' do
    ...
    it 'should return Internal Service Error, if execute trade service is unavailable', vcr: {cassette_name: 'execute_trade_service_unavailable'} do
      get "/member/#{MEMBER_ID}/execute_advance/#{operation}/#{amount}/#{advance_type}/#{advance_term}/#{rate}/#{signer}"
