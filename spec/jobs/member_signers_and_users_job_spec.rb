...
RSpec.describe MemberSignersAndUsersJob, type: :job do
  ...
  before do
    ...
    allow_any_instance_of(MembersService).to receive(:signers_and_users).with(member_id).and_return(results)
  ...
  it 'should call `MembersService#signers_and_users`' do
    expect_any_instance_of(MembersService).to receive(:signers_and_users).with(member_id)
    run_job
  end

  it 'should fail the job if the MembersService call fails' do
    allow_any_instance_of(MembersService).to receive(:signers_and_users).with(member_id).and_return(nil)
