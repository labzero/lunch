...
describe ReportsHelper do
  {
    ...
    securities_services_line_item: [:number, 'securities-services-line-item-number']
    ...
    describe "`#{method.to_s}`" do
      ...
      it 'calls translation_with_span with the i18n string it was passed' do
        expect(helper).to receive(:translation_with_span).with(i18n_string, anything, anything, anything, anything )
        call_method
      end
      it "calls translation_with_span with `#{arguments.first}` as the span_key arg" do
        expect(helper).to receive(:translation_with_span).with(anything, arguments.first, anything, anything, anything )
        call_method
      end
      it 'calls translation_with_span with the span_value it was passed as the span_value arg' do
        expect(helper).to receive(:translation_with_span).with(anything, anything, span_val, anything, anything )
        call_method
      end
      it "calls translation_with_span with `#{arguments.last}` as the klass arg" do
        expect(helper).to receive(:translation_with_span).with(anything, anything, anything, arguments.last, anything )
        call_method
      end
      it 'calls translation_with_span with any other substitutions it was given as the subs arg' do
        expect(helper).to receive(:translation_with_span).with(anything, anything, anything, anything, other_arg_hash )
        helper.send(method, i18n_string, span_val, other_arg_hash)
      end
      it 'calls translation_with_span with an empty hash for the subs arg if it was not passed any substitutions' do
        expect(helper).to receive(:translation_with_span).with(anything, anything, anything, anything, {} )
        call_method
      end
    end
  end
  ...
  describe 'the `translation_with_span` private method' do
    ...
    let(:call_method) { helper.send(:translation_with_span, i18n_string, span_key, span_value, klass) }
    ...
    it 'sends the given string to I18n interpolation with any other args that were passed' do
      ...
      helper.send(:translation_with_span, i18n_string, span_key, span_value, klass, other_arg_hash)
    end
