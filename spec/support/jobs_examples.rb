RSpec.shared_examples 'a report that can be downloaded' do |method, download_options|
  ...
  jobs.each do |format, job|
    describe "downloading a #{format.upcase}" do
      ...
      before do
        ...
        allow(job).to receive(:perform_later).and_return(active_job)
        allow(controller).to receive(:current_user).and_return(current_user)
        allow(controller).to receive(:fhlb_report_date_numeric)
      end

      it "enqueues a report #{format} job when the requested `export_format` is `#{format}`" do
        expect(job).to receive(:perform_later).with(member_id, method.to_s, any_args).and_return(active_job)
        get method.to_sym, export_format: format
      end
...
RSpec.shared_examples 'a job that makes service calls' do |service, methods|
  ...
  before do
    ...
    methods.each do |method|
      allow(service_instance).to receive(method)
    end
  end

  methods.each do |method|
    it "calls `#{service.to_s}##{method.to_s}`" do
      expect(service_instance).to receive(method)
      run_job
    end
  end
